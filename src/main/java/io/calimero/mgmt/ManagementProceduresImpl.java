...
public class ManagementProceduresImpl implements ManagementProcedures
{
	...
	public List<byte[]> scanSerialNumbers(final int medium) throws KNXException, InterruptedException
	{
		synchronized (mc) {
			...
			try (Destination dst = mc.createDestination(new IndividualAddress(0, medium, 0xff), true)) {
				...
				return ((ManagementClientImpl) mc).readProperty(dst, 0, PID.SERIAL_NUMBER, 1, 1, false);
			}
			catch (final KNXTimeoutException e) {
				// no response is fine
			}
		...
		throws KNXException, InterruptedException
	{
		...
		try {
			...
			mc.writeProperty(d, DEVICE_OBJECT_INDEX, PID.PROGMODE, 1, 1,
		...
		throws KNXException, InterruptedException
	{
		...
		if (!extMemoryServices && verifyByServer) {
			...
			ctrl[0] |= 0x04;
			mc.writeProperty(d, DEVICE_OBJECT_INDEX, PID.DEVICE_CONTROL,
				1, 1, ctrl);
		}
